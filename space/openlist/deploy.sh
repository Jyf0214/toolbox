...
mkdir -p /app /opt/tunnel /opt/openlist/data "${LOG_DIR}" "${DATA_DIR}" /home/appuser/.config/rclone
...
echo "5. Generating sync tool configuration TEMPLATES..."
...
# litestream 模板
cat << EOF > /etc/litestream.yml.template
dbs:
